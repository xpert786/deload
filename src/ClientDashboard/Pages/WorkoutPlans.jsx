            ...
            <div className="bg-white rounded-2xl p-5 sm:p-6 border border-gray-100">
              <div className="mb-4 flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-sm font-medium text-[#FFFFFF]  font-[Inter] !bg-[#003F8F] px-5 py-2 rounded-lg cursor-pointer"
                >
                  View all
                </button>
              </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2 mb-4">
            <button
              ...
              className="flex items-center gap-1 text-sm text-white  transition-colors bg-[#003F8F] px-3 py-2 rounded-lg cursor-pointer"
            >
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100">
            <div className="mb-6">
              <h2 className="text-xl font-medium text-[#003F8F] font-[Poppins]">Workouts History</h2>
              <p className="text-sm text-gray-500 font-[Inter]">Your workout history and performance</p>
            </div>

            {workoutsHistory.workouts && workoutsHistory.workouts.length > 0 ? (
              <div className="space-y-3">
                {workoutsHistory.workouts.map((item, idx) => {
                  const isCompleted = item.status === 'completed' || item.status_display === 'Completed';
                  const isIncomplete = item.status === 'incomplete' || item.status_display === 'Incomplete';

                  return (
                    <div key={item.id || idx} className="bg-white rounded-xl border border-gray-200 p-4 transition-colors hover:bg-gray-50">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        {/* Left: Number + Details */}
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 rounded-full border border-[#003F8F] text-[#003F8F] font-semibold text-sm flex items-center justify-center bg-white flex-shrink-0">
                            {idx + 1}
                          </div>
                          <div>
                            <p className="font-semibold text-[#003F8F] text-base font-[Inter]">{item.workout_name}</p>
                            <p className="text-sm text-gray-500 font-[Inter]">{item.date_display || item.date}</p>
                          </div>
                        </div>

                        {/* Right: Duration + Status */}
                        <div className="flex items-center gap-6 self-start sm:self-center ml-14 sm:ml-0">
                          {isCompleted && item.duration_display && (
                            <div className="text-right hidden sm:block">
                              <p className="text-sm font-bold text-[#003F8F] font-[Inter]">{item.duration_display}</p>
                              <p className="text-xs text-gray-500 font-[Inter]">Duration</p>
                            </div>
                          )}

                          <span
                            className={`text-sm font-medium font-[Inter] ${isCompleted ? 'text-[#003F8F]' :
                              isIncomplete ? 'text-[#FB923C]' : 'text-gray-500'
                              }`}
                          >
                            {item.status_display || item.status}
                          </span>
              ...
              <div className="text-center py-12">
                <p className="text-gray-500 font-[Inter]">No workout history found.</p>
              </div>
            )}
          </div>
        </div>
