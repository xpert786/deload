// CoachThreadDetail Component - Chat view specifically for coaches
import React, { useState, useEffect, useRef, useCallback } from 'react';
import { useWebSocket } from '../../hooks/useWebSocket';
import { getThreadMessages } from '../../services/threadsApi';

const CoachThreadDetail = ({ thread, currentUserId, onBack, onThreadUpdate }) => {
  // Normalize currentUserId - extract numeric part if it's a string like "coach_21"
  ...
  // For coach view: current user is always the coach, other user is always the client
  const clientUser = {
  ...
  const handleWebSocketMessage = useCallback((data) => {
    ...
    } else if (data.type === 'typing' && data.sender_id === clientUser.id) {
      setIsTyping(data.is_typing);
    }
  }, [thread.id, clientUser.id, onThreadUpdate]);
  ...
  }, [isConnected, sendMessage, clientUser.id]);
  ...
  }, [messages, effectiveCurrentUserId, clientUser.id, isConnected, sendMessage]);
    ...
    <div className="flex-1 flex flex-col bg-white rounded-xl overflow-hidden shadow-sm">
      ...
      <div className="bg-[#003F8F] text-white p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="w-10 h-10 rounded-full flex items-center justify-center bg-[#003F8F]">
            ...
            <span className={`text-white text-sm font-semibold ${clientUser.photo ? 'hidden' : 'flex'}`}>
              {getInitials(clientUser.name)}
            </span>
          </div>
          <div>
            <p className="font-semibold font-[Inter]">{clientUser.name}</p>
            {isTyping && (
      ...
      >
        ...
        <div className="space-y-4">
          {messages.map((msg) => {
            // For coach view: coach messages are "own", client messages are "received"
            ...
            // Message is from coach (current user) if sender matches coach ID
            ...
            const isOwnMessage = 
              ...
              (msgSenderNumeric !== null && threadCoachIdNumeric !== null && msgSenderNumeric === threadCoachIdNumeric) ||
              // Method 2: Direct comparison
              (msg.sender_id == effectiveCurrentUserId || msg.sender == effectiveCurrentUserId) ||
              (msg.sender_id == (thread.coach || thread.coach_id) || msg.sender == (thread.coach || thread.coach_id)) ||
              ...
              // Method 4: Check sender_name matches coach name (fallback)
              (msg.sender_name && coachUser.name && 
               msg.sender_name.toLowerCase().trim() === coachUser.name.toLowerCase().trim());
            
            // Determine if sender is coach or client based on thread IDs
            const senderIsCoach = 
              (msgSenderNumeric !== null && threadCoachIdNumeric !== null && msgSenderNumeric === threadCoachIdNumeric) ||
              (msg.sender_id == (thread.coach || thread.coach_id) || msg.sender == (thread.coach || thread.coach_id)) ||
              (msg.sender_id != null && (thread.coach || thread.coach_id) != null &&
               String(msg.sender_id).replace(/\D/g, '') === String(thread.coach || thread.coach_id).replace(/\D/g, ''));
            
            // ALWAYS use message sender_name if available (most reliable)
            // Fallback to determining from thread data
            const senderName = msg.sender_name || (senderIsCoach ? coachUser.name : clientUser.name);
            const senderAvatar = senderIsCoach ? coachUser.photo : clientUser.photo;
            
            // For own messages (coach), ALWAYS use coach info; for received (client), use client info
            // If message has sender_name that matches coach, use it; otherwise use coachUser.name
            const displayName = isOwnMessage 
              ? (msg.sender_name && msg.sender_name.toLowerCase().trim() === coachUser.name.toLowerCase().trim() 
                 ? msg.sender_name 
                 : coachUser.name)  // Always use coach name for own messages
              : clientUser.name;
            const displayAvatar = isOwnMessage 
              ? coachUser.photo  // Always use coach avatar for own messages
              : clientUser.photo;
            
            // Debug logging
            console.log('🔍 CoachThreadDetail - Message check:', {
              messageId: msg.id,
...
export default CoachThreadDetail;
